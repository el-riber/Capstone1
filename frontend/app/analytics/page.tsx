...
export default function AnalyticsPage() {
  ...
  const calculateCorrelations = () => {
    ...
    const sleepHours = entries
      .filter((e) => (e.sleep_hours ?? 0) > 0)
      .map((e) => Number(e.sleep_hours ?? 0));
    const sleepMoods = entries
      .filter((e) => (e.sleep_hours ?? 0) > 0)
      .map((e) => Number(e.mood ?? 0));
    const energy = entries.map((e) =>
      Number((e as any).energy_level ?? e.mood ?? 0)
    );
    const social = entries.map((e) =>
      Number((e as any).social_interaction ?? 3)
    );

    return {
      sleepMoodCorrelation:
        sleepHours.length > 2 ? pearson(sleepHours, sleepMoods) : 0,
                ...
                <div className="bg-white rounded-2xl shadow-md p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="bg-green-50 p-4 rounded-lg">
                      <div className="text-sm text-green-600 font-medium">Average Mood</div>
                    ...
                    <div className="bg-purple-50 p-4 rounded-lg">
                      <div className="text-sm text-purple-600 font-medium">Entries Count</div>
                  ...
                  <div className="space-y-2">
                    {(() => {
                      ...
                      return data.map((t) => {
                        ...
                        return (
                          <div key={t.trigger} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                            <span className="font-medium">{t.trigger}</span>
                            <div className="flex items-center space-x-2">
                              <span className="text-sm text-gray-600">{t.count} times</span>
                              <div className="w-20 bg-gray-200 rounded-full h-2">
                                <div className="bg-red-500 h-2 rounded-full" style={{ width: `${width}%` }} />
                              </div>
                            </div>
                          </div>
                        );
                      ...
                      <div key={ep.id} className={`border ${styles.border} ${styles.bg} rounded-lg p-4`}>
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex space-x-2">
                            ...
                            <button onClick={() => deleteEpisode(ep.id!)} className="p-1 text-red-600 hover:text-red-800">
                          ...
                          <div className="mt-3">
                            ...
                            <div className="flex flex-wrap gap-1">
                              {ep.symptoms.slice(0, 5).map((s) => (
                                <span key={s} className={`px-2 py-1 ${styles.chipBg} ${styles.chipText} text-xs rounded`}>
                                  {s}
                                </span>
                              ))}
                      ...
                      {(() => {
                        ...
                        return monthlyAverages.map((m) => (
                          <div key={m.label} className="flex items-center justify-between">
                            ...
                            <span className="text-sm font-medium text-blue-900">{m.avg.toFixed(1)}/5</span>
                          </div>
                        ));
                      })()}
                    </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      onChange={(e) => setEpisodeFormData((f) => ({ ...f, hospitalization: e.target.checked }))}
                      className="h-4 w-4"
                    />
                    <label htmlFor="hosp" className="text-sm text-gray-700">
                      Required hospitalization
                    </label>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Medication changes</label>
                    <input
                      type="text"
                      value={episodeFormData.medication_changes || ""}
                      onChange={(e) => setEpisodeFormData((f) => ({ ...f, medication_changes: e.target.value }))}
                      className="w-full border rounded-md px-3 py-2"

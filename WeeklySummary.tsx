...
export default function WeeklySummary() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const u = userData?.user ? { id: userData.user.id, email: userData.user.email ?? undefined } : null;
      setUser(u);
    })();
  }, []);

  // 2) Compute crisis flags whenever entries change
  useEffect(() => {
    if (moodEntries.length > 0) {
      const flags = CrisisDetectionService.analyzeMoodPatterns(moodEntries);
      const active = flags.filter((f) => !dismissedFlags.has(f.type));
      setCrisisFlags(active);
    } else {
      setCrisisFlags([]);
    }
  }, [moodEntries, dismissedFlags]);

  // 3) Main refresh action
  const generateSummary = useCallback(async () => {
    ...
    try {
      ...
      // keep local user in sync (useful for the export footer)
      setUser({ id: user_id, email: user_email ?? undefined });
      ...
      if (!entries || entries.length === 0) {
        setSummary(
          "No mood entries found for the past week. Start tracking your mood daily to get personalized insights!"
        );
  ...
  // 4) Run once after user loads
  useEffect(() => {
    if (user?.id) generateSummary();
  }, [user?.id, generateSummary]);

  // 5) Refresh when a new entry is saved elsewhere in the app
  useEffect(() => {
    const onSaved = () => generateSummary();
    window.addEventListener("mood-saved", onSaved);
    return () => window.removeEventListener("mood-saved", onSaved);
  }, [generateSummary]);

  // 6) Refresh when tab regains focus
  useEffect(() => {
    const onFocus = () => generateSummary();
    window.addEventListener("focus", onFocus);
    return () => window.removeEventListener("focus", onFocus);
  }, [generateSummary]);

  // 7) Supabase Realtime: refresh on INSERTs for this user (both tables)
  useEffect(() => {
    if (!user?.id) return;

    const chan = supabase
      .channel("weekly-summary-realtime")
      .on(
        "postgres_changes",
        {
          event: "INSERT",
  ...
  const getAverageMood = (): number => {
    ...
    // NOTE: if your scale is 1..8, this will show like "5.4/8" below.
  };

  const getMoodTrend = (): "up" | "down" | "stable" => {
    if (moodEntries.length < 2) return "stable";
    const recent = moodEntries.slice(0, 3).reduce((acc, e) => acc + e.mood, 0) / Math.min(3, moodEntries.length);
    const older = moodEntries.slice(-3).reduce((acc, e) => acc + e.mood, 0) / Math.min(3, moodEntries.length);
    ...
    const pdfContent = `
      ...
      <body>
        <div class="header">
          ...
          <p class="subtitle">${user?.email || "Personal Report"} - Generated on ${new Date().toLocaleDateString()}</p>
        ...
        <div class="section">
          ...
          ${moodEntries.slice(0, 5).map(e => `
            <div class="entry">
              <div class="entry-date">${new Date(e.created_at).toLocaleDateString()} ${e.emoji}</div>
        ...
        <div class="disclaimer">
          <p class="disclaimer-text">
            This report is generated for personal reflection purposes only. It is not a substitute for professional mental health advice, diagnosis, or treatment.
          </p>
        </div>
